...
category: ["Authorization", ".NET Auth Internals"]
...
- Implementing our own OAuth service could use Google auth, Facebook auth, cookies, return JWT, etc.
...
3. External auth (**Google**, **Facebook**, **Microsoft**) means emitting OAuth flow with configured API keys and secrets and ensuring external auth result is successful.
...
For now, it could be not that clear why we need this **IAuthenticationRequestHandler** if we already have **IAuthenticationHandler**. We will discuss this along with **AuthenticationMiddleware**.
...
[https://github.com/dotnet/aspnetcore/blob/main/src/Http/Authentication.Core/src/AuthenticationHandlerProvider.cs](https://github.com/dotnet/aspnetcore/blob/main/src/Http/Authentication.Core/src/AuthenticationHandlerProvider.cs)
...
**Line 55-63**: we try each scheme that is tied to the **IAuthenticationRequestHandler** handler type to do auth process. If a particular request handler returned true then it means no need to continue auth, we return.
...
A good example of the handler that is implementing **IAuthenticationRequestHandler** is **RemoteAuthenticationHandler**([https://github.com/dotnet/aspnetcore/blob/main/src/Security/Authentication/Core/src/RemoteAuthenticationHandler.cs](https://github.com/dotnet/aspnetcore/blob/main/src/Security/Authentication/Core/src/RemoteAuthenticationHandler.cs)).
